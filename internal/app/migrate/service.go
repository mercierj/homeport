...
func resolvePortConflict(portMapping string, usedPorts map[string]bool, portCounter map[int]int) string {
	...
	if usedPorts[hostPort] {
		...
		basePort := 0
		_, _ = fmt.Sscanf(hostPort, "%d", &basePort)
		if basePort > 0 {
...
`, titleCase(config.Provider), titleCase(config.Provider), titleCase(config.Provider),
		titleCase(config.Provider), doc, titleCase(config.Provider))
}
