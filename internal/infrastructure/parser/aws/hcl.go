...
// HCLParser parses Terraform HCL (.tf) files for AWS resources.
type HCLParser struct{}

// NewHCLParser creates a new AWS HCL parser.
...
// Validate checks if the path contains valid Terraform HCL files with AWS resources.
...
// Parse parses Terraform HCL files and returns AWS infrastructure.
...
// hasAWSConfig checks if a file contains AWS configuration.
func (p *HCLParser) hasAWSConfig(path string) bool {
	...
	return strings.Contains(content, `provider "aws"`) ||
		strings.Contains(content, `resource "aws_`) ||
		strings.Contains(content, `data "aws_`)
}
...
func (p *HCLParser) parseResourceBlock(resourceType, resourceName string, block *hcl.Block) *resource.Resource {
	...
	resType := mapAWSTerraformType(resourceType)
	...
	for attrName, attr := range attrs {
		...
		if !diags.HasErrors() {
			...
			if attrName == "availability_zone" {
				if strVal, ok := res.Config[attrName].(string); ok && len(strVal) > 1 {
					res.Region = strVal[:len(strVal)-1]
				}
			}
