...
// HCLParser parses Terraform HCL (.tf) files for GCP resources.
type HCLParser struct{}

// NewHCLParser creates a new GCP HCL parser.
...
// Validate checks if the path contains valid Terraform HCL files with GCP resources.
...
// Parse parses Terraform HCL files and returns GCP infrastructure.
...
// hasGCPConfig checks if a file contains GCP configuration.
func (p *HCLParser) hasGCPConfig(path string) bool {
	...
	content := string(data)
	return strings.Contains(content, `provider "google"`) ||
		strings.Contains(content, `resource "google_`) ||
		strings.Contains(content, `data "google_`)
}
...
func (p *HCLParser) parseResourceBlock(resourceType, resourceName string, block *hcl.Block) *resource.Resource {
	...
	resType := mapGCPTerraformType(resourceType)
	...
	for attrName, attr := range attrs {
		...
		if !diags.HasErrors() {
			...
			if attrName == "labels" {
				if labels, ok := res.Config[attrName].(map[string]interface{}); ok {
