...
func TestAppEngineConformanceManagedAToZ(t *testing.T) {
	result, err := NewAppEngineMapper().Map(context.Background(), &resource.AWSResource{
		ID:     "projects/demo/apps/demo-app",
	...
	if len(result.ManualSteps) != 0 {
		t.Fatalf("manual steps = %#v, want generated App Engine migration", result.ManualSteps)
	}
	if result.DockerService.Deploy == nil || result.DockerService.Deploy.Replicas < 2 {
		t.Fatalf("service does not provision HA App Engine target: %#v", result.DockerService)
	}
	for _, file := range []string{"appengine/demo-app/Dockerfile", "appengine/demo-app/app.yaml", "config/appengine/app-change.env", "config/appengine/generated-client.patch"} {
		if _, ok := result.Configs[file]; !ok {
			t.Fatalf("missing config %s", file)
		}
	}
	appEnv := string(result.Configs["config/appengine/app-change.env"])
	for _, want := range []string{"APP_CHANGE_MODE=generated_patch", "SOURCE_APP_ENGINE_APP=demo-app", "TARGET_APP_PLATFORM=docker"} {
		if !strings.Contains(appEnv, want) {
			t.Fatalf("app-change env missing %q:\n%s", want, appEnv)
		}
	}
	for _, file := range []string{"export_appengine_config.sh", "provision_appengine_container.sh", "migrate_appengine_app.sh", "validate_appengine_container.sh", "backup_appengine_config.sh", "cutover_appengine_routes.sh"} {
		if _, ok := result.Scripts[file]; !ok {
			t.Fatalf("missing script %s", file)
		}
	}
	for id, stepType := range map[string]domainrunbook.StepType{
		"export-appengine-config":       domainrunbook.StepTypeCommand,
		"provision-appengine-container": domainrunbook.StepTypeCommand,
		"migrate-appengine-app":         domainrunbook.StepTypeCommand,
		"validate-appengine-container":  domainrunbook.StepTypeCommand,
		"backup-appengine-config":       domainrunbook.StepTypeCommand,
		"cutover-appengine-routes":      domainrunbook.StepTypeAPICall,
		"rollback-appengine-source":     domainrunbook.StepTypeRollback,
	} {
		if !hasAppEngineRunbookStep(result, id, stepType) {
			t.Fatalf("missing %s runbook step: %#v", id, result.RunbookSteps)
		}
	}
}

func hasAppEngineRunbookStep(result *mapper.MappingResult, id string, stepType domainrunbook.StepType) bool {
