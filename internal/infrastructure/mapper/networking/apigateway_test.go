	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, result *mapper.MappingResult) {
				...
				if result.DockerService.Labels["cloudexit.source"] != "aws_api_gateway" {
					t.Errorf("Expected source label to be aws_api_gateway, got %s", result.DockerService.Labels["cloudexit.source"])
				}
		...
		},
		{
			...
			check: func(t *testing.T, result *mapper.MappingResult) {
				...
				if result.DockerService.Labels["cloudexit.api_name"] != "fallback-api" {
					t.Errorf("Expected api_name label to be fallback-api, got %s", result.DockerService.Labels["cloudexit.api_name"])
				}
			},
		},
		{
