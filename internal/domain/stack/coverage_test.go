...
// TestResourceMappingCoverage verifies that all resource types are mapped.
...
func TestResourceMappingCount(t *testing.T) {
	...
	expectedGCP := 53
	expectedAzure := 39
	expectedTotal := expectedAWS + expectedGCP + expectedAzure // 155
	...
	// Verify CategoryMapping also has the expected entries (our source of truth)
...
func TestGetStackTypeForResource(t *testing.T) {
	tests := []struct {
		...
		{resource.TypeMSKCluster, StackTypeMessaging, true},
		{resource.TypeSESIdentity, StackTypeMessaging, true},
		{resource.TypeStepFunctionsStateMachine, StackTypeMessaging, true},
		{resource.TypeMQBroker, StackTypeMessaging, true},
		{resource.TypeIoTThing, StackTypeMessaging, true},
		...
		{resource.TypeXRaySamplingRule, StackTypeObservability, true},
		...
		{resource.TypeRekognitionCollection, StackTypePassthrough, true},
		{resource.TypeComprehendClassifier, StackTypePassthrough, true},
		...
		{resource.TypeAppMeshMesh, StackTypePassthrough, true},
		{resource.TypeIAMRole, StackTypePassthrough, true},
		{resource.TypeACMCertificate, StackTypePassthrough, true},
		{resource.TypeWAFWebACL, StackTypePassthrough, true},
		{resource.TypeShieldProtection, StackTypePassthrough, true},
		{resource.TypeSecurityHubAccount, StackTypePassthrough, true},
		{resource.TypeAWSConfigRule, StackTypePassthrough, true},
		{resource.TypeOrganizationsOrganization, StackTypePassthrough, true},
		{resource.TypeControlTowerControl, StackTypePassthrough, true},
		{resource.TypeLakeFormationPermissions, StackTypePassthrough, true},
		{resource.TypeQuickSightDashboard, StackTypeDatabase, true},
		...
		{resource.TypeBigQuery, StackTypeDatabase, true},
		...
		{resource.TypeEventarcTrigger, StackTypeMessaging, true},

		// GCP Observability
		{resource.TypeCloudLoggingSink, StackTypeObservability, true},
		{resource.TypeCloudMonitoringAlertPolicy, StackTypeObservability, true},
		{resource.TypeCloudMonitoringDashboard, StackTypeObservability, true},
		{resource.TypeCloudTraceService, StackTypeObservability, true},
		...
		{resource.TypeArtifactRegistryRepository, StackTypePassthrough, true},
		{resource.TypeCloudBuildTrigger, StackTypePassthrough, true},
		{resource.TypeDataprocCluster, StackTypePassthrough, true},
		...
		{resource.TypeApigeeOrganization, StackTypePassthrough, true},
		...
		// Azure AI/ML
		{resource.TypeAzureFoundryOpenAI, StackTypePassthrough, true},
		...
		{resource.TypeAzureIoTHub, StackTypeMessaging, true},
		{resource.TypeLogicApp, StackTypeMessaging, true},
		{resource.TypeAzureDataFactory, StackTypeMessaging, true},
		{resource.TypeAzureDatabricks, StackTypePassthrough, true},

		// Azure Observability
		{resource.TypeAzureLogAnalytics, StackTypeObservability, true},
		...
		{resource.TypeAzureContainerApp, StackTypePassthrough, true},
		{resource.TypeAzureContainerRegistry, StackTypePassthrough, true},
		...
		{resource.TypeAppInsights, StackTypeObservability, true},
	}
...
func TestGetResourceMappingStats(t *testing.T) {
	...
	if stats.TotalTypes != 155 {
		t.Errorf("TotalTypes = %d, expected 155", stats.TotalTypes)
	}

	if stats.CoveredTypes != 155 {
		t.Errorf("CoveredTypes = %d, expected 155", stats.CoveredTypes)
	}
	...
	if stats.ByProvider[resource.ProviderAWS] != 63 {
		t.Errorf("AWS count = %d, expected 63", stats.ByProvider[resource.ProviderAWS])
	}
	if stats.ByProvider[resource.ProviderGCP] != 53 {
		t.Errorf("GCP count = %d, expected 53", stats.ByProvider[resource.ProviderGCP])
	}
	if stats.ByProvider[resource.ProviderAzure] != 39 {
		t.Errorf("Azure count = %d, expected 39", stats.ByProvider[resource.ProviderAzure])
	}
}
...
func TestStackTypeDistribution(t *testing.T) {
	...
	expectedMinimums := map[StackType]int{
		...
		StackTypeMessaging:     16, // SQS, SNS, MQ, IoT, EventBridge, Kinesis, SES, Step Functions, PubSub, ServiceBus, etc.
		StackTypeStorage:       7,  // S3, EFS, GCS, Filestore, BlobStorage, AzureStorageAcct, AzureFiles
		StackTypeObservability: 4,  // CloudWatch alarms, logs, dashboards, X-Ray
		...
		StackTypePassthrough:   36, // VMs, containers, devops, IaC, networking, service mesh, ML endpoints, WAF, data governance, etc.
	}
...
func TestAllResourceTypes(t *testing.T) {
	...
	if len(types) != 155 {
		t.Errorf("AllResourceTypes returned %d types, expected 155", len(types))
	}
...
func TestNetworkingArePassthrough(t *testing.T) {
	networkTypes := []resource.Type{
		...
		resource.TypeApigeeOrganization,
		// Azure
