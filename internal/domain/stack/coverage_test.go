...
func TestResourceMappingCount(t *testing.T) {
	...
	expectedGCP := 53
	expectedAzure := 29
	expectedTotal := expectedAWS + expectedGCP + expectedAzure // 145
...
func TestGetResourceMappingStats(t *testing.T) {
	...
	if stats.TotalTypes != 145 {
		t.Errorf("TotalTypes = %d, expected 145", stats.TotalTypes)
	}

	if stats.CoveredTypes != 145 {
		t.Errorf("CoveredTypes = %d, expected 145", stats.CoveredTypes)
	}
...
func TestAllResourceTypes(t *testing.T) {
	...
	if len(types) != 145 {
		t.Errorf("AllResourceTypes returned %d types, expected 145", len(types))
	}
