...
func TestResourceMappingCount(t *testing.T) {
	...
	expectedGCP := 50
	expectedAzure := 29
	expectedTotal := expectedAWS + expectedGCP + expectedAzure // 142
...
func TestGetResourceMappingStats(t *testing.T) {
	...
	if stats.TotalTypes != 142 {
		t.Errorf("TotalTypes = %d, expected 142", stats.TotalTypes)
	}

	if stats.CoveredTypes != 142 {
		t.Errorf("CoveredTypes = %d, expected 142", stats.CoveredTypes)
	}
	...
	if stats.ByProvider[resource.ProviderGCP] != 50 {
		t.Errorf("GCP count = %d, expected 50", stats.ByProvider[resource.ProviderGCP])
	}
...
func TestAllResourceTypes(t *testing.T) {
	...
	if len(types) != 142 {
		t.Errorf("AllResourceTypes returned %d types, expected 142", len(types))
	}
