...
func TestResourceMappingCount(t *testing.T) {
	...
	expectedGCP := 49
	expectedAzure := 29
	expectedTotal := expectedAWS + expectedGCP + expectedAzure // 141
...
func TestGetResourceMappingStats(t *testing.T) {
	...
	if stats.TotalTypes != 141 {
		t.Errorf("TotalTypes = %d, expected 141", stats.TotalTypes)
	}

	if stats.CoveredTypes != 141 {
		t.Errorf("CoveredTypes = %d, expected 141", stats.CoveredTypes)
	}
	...
	if stats.ByProvider[resource.ProviderGCP] != 49 {
		t.Errorf("GCP count = %d, expected 49", stats.ByProvider[resource.ProviderGCP])
	}
...
func TestAllResourceTypes(t *testing.T) {
	...
	if len(types) != 141 {
		t.Errorf("AllResourceTypes returned %d types, expected 141", len(types))
	}
