...
func TestResourceMappingCount(t *testing.T) {
	...
	expectedGCP := 52
	expectedAzure := 29
	expectedTotal := expectedAWS + expectedGCP + expectedAzure // 144
...
func TestGetResourceMappingStats(t *testing.T) {
	...
	if stats.TotalTypes != 144 {
		t.Errorf("TotalTypes = %d, expected 144", stats.TotalTypes)
	}

	if stats.CoveredTypes != 144 {
		t.Errorf("CoveredTypes = %d, expected 144", stats.CoveredTypes)
	}
	...
	if stats.ByProvider[resource.ProviderGCP] != 52 {
		t.Errorf("GCP count = %d, expected 52", stats.ByProvider[resource.ProviderGCP])
	}
...
func TestAllResourceTypes(t *testing.T) {
	...
	if len(types) != 144 {
		t.Errorf("AllResourceTypes returned %d types, expected 144", len(types))
	}
