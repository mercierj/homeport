...
- Actions explicitly not supported first: Bigtable console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `bigtableadmin.projects.instances.create` and its paired read/list calls.
...
- Error mapping: translate `gcp/bigtable` backend auth, missing `google_bigtable_instance`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
