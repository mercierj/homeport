...
- Actions explicitly not supported first: Dataplex console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `dataplex.projects.locations.lakes.create` and its paired read/list calls.
- Ledger resource types: source Dataplex resource model
- First concrete resource model to add: source Dataplex resource with import id, region/location, labels/tags, backend target id, lifecycle state, and owner principal.
- Provider errors: map Dataplex authorization failures to GCP access-denied codes, missing `source Dataplex resource model` records to not-found codes, duplicate imports to conflict/already-exists, invalid mapped fields to validation errors, backend saturation to throttle/quota responses, and unexpected `gcp/dataplex` failures to provider internal-error shapes with request ids.
- Pagination/idempotency/tags: list/read calls expose provider tokens where the API has them; mutating calls persist idempotency keys or operation ids; tags/labels round-trip on the source Dataplex resource model.

## Backend

- Backend: Apache Atlas.
- Storage and metadata: Dataplex state lives in `Apache Atlas`; HomePort stores provider identifiers for `source Dataplex resource model`, source import ids, authz bindings, generated artifact checksums, backup references, and audit events.
...
- Context: evaluate Dataplex calls with tenant/project/account, provider region/location, `projects/{project}/locations/{location}/dataplex/{id}`, source IP, request id, user agent, tags/labels on `source Dataplex resource model`, credential age, and MFA/managed-identity claims when the source provider supplies them.
- Evaluation: call `Authorize(principal, action, resource, context)` before each mutating operation and each data-plane read/write.
- Conditions: support exact/wildcard matches for the listed Dataplex actions, `projects/{project}/locations/{location}/dataplex/{id}` prefix checks, tag/label equality on `source Dataplex resource model`, requested region/location, source IP CIDR, time window, and principal attributes.
...
- Request mapping: Dataplex provider names, locations, tags/labels, and request bodies map to HomePort `source Dataplex resource model` records and `Apache Atlas` configuration; backend-only knobs are omitted from provider responses.
- Response mapping: return Dataplex provider ids, `source Dataplex resource model` lifecycle state, operation ids, etags/versions where the source API exposes them, list pagination tokens, and HomePort audit timestamps without exposing backend-only fields.
- Error mapping: translate `gcp/dataplex` backend auth, missing `source Dataplex resource model`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
...
- Fixture import covers the source Dataplex resource model from `gcp/dataplex`.
