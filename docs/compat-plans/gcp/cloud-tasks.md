...
Expose the smallest GCP Cloud Tasks-compatible surface needed to migrate the ledger resources to `Celery and Redis with generated Cloud Tasks client patch` without claiming managed-service parity outside the contract tests below.
...
- Actions explicitly not supported first: Cloud Tasks console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `cloudtasks.projects.locations.queues.create` and its paired read/list calls.
...
- Backend: Celery and Redis with generated Cloud Tasks client patch.
- Storage and metadata: Cloud Tasks state lives in `Celery and Redis with generated Cloud Tasks client patch`; HomePort stores provider identifiers for `google_cloud_tasks_queue`, source import ids, authz bindings, generated artifact checksums, backup references, and audit events.
- Secrets/keys/tokens: issue HomePort-scoped credentials from the identity/secrets layer; store provider source credentials only as encrypted migration inputs.
- Runtime/provisioning: provision `Celery and Redis with generated Cloud Tasks client patch` with generated `artifacts/compat/gcp/cloud-tasks/backend.yaml`, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `gcp/cloud-tasks`.
...
- Request mapping: Cloud Tasks provider names, locations, tags/labels, and request bodies map to HomePort `google_cloud_tasks_queue` records and `Celery and Redis with generated Cloud Tasks client patch` configuration; backend-only knobs are omitted from provider responses.
...
- Error mapping: translate `gcp/cloud-tasks` backend auth, missing `google_cloud_tasks_queue`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.

## Generated Artifacts

- `artifacts/compat/gcp/cloud-tasks/backend.yaml` for `Celery and Redis with generated Cloud Tasks client patch` runtime, network, persistence, health check, and backup policy.
