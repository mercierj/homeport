...
- Actions explicitly not supported first: Apigee console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `apigee.organizations.get` and its paired read/list calls.
...
- Error mapping: translate `gcp/apigee` backend auth, missing `google_apigee_organization`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
