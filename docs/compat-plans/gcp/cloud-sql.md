...
- Actions explicitly not supported first: Cloud SQL console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `sqladmin.instances.insert` and its paired read/list calls.
...
- Runtime/provisioning: provision `PostgreSQL or MySQL` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `gcp/cloud-sql`.
...
- Error mapping: translate `gcp/cloud-sql` backend auth, missing `google_sql_database_instance`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
