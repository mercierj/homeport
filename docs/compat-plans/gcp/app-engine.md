...
- Actions explicitly not supported first: App Engine console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `appengine.apps.get` and its paired read/list calls.
...
- Error mapping: translate `gcp/app-engine` backend auth, missing `google_app_engine_application`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
