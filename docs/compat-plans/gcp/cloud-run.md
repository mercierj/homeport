...
- Actions explicitly not supported first: Cloud Run console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `run.projects.locations.services.create` and its paired read/list calls.
...
- Runtime/provisioning: provision `Knative Serving` with generated `artifacts/compat/gcp/cloud-run/backend.yaml`, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `gcp/cloud-run`.
...
- Error mapping: translate `gcp/cloud-run` backend auth, missing `google_cloud_run_service`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
