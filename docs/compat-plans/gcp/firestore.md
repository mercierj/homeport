...
- Actions explicitly not supported first: Firestore console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `firestore.projects.databases.create` and its paired read/list calls.
...
- Runtime/provisioning: provision `FerretDB with document-store compatibility review` with generated `artifacts/compat/gcp/firestore/backend.yaml`, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `gcp/firestore`.
...
- Error mapping: translate `gcp/firestore` backend auth, missing `google_firestore_database`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
