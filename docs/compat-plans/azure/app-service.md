...
- Actions explicitly not supported first: App Service console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Web/sites/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `Dokku or Cloud Foundry buildpacks` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/app-service`.
...
- Error mapping: translate `azure/app-service` backend auth, missing `azurerm_app_service`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
