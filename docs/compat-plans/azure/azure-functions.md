...
- Actions explicitly not supported first: Azure Functions console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Web/sites/functions/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `OpenFaaS` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/azure-functions`.
...
- Error mapping: translate `azure/azure-functions` backend auth, missing `azurerm_function_app`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
