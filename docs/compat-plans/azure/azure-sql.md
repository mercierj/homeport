...
- Actions explicitly not supported first: Azure SQL console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Sql/servers/databases/read` and its paired read/list calls.
...
- Backend: PostgreSQL with TDS compatibility review.
...
- Runtime/provisioning: provision `PostgreSQL with TDS compatibility review` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/azure-sql`.
...
- Error mapping: translate `azure/azure-sql` backend auth, missing `azurerm_mssql_database`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
