...
- Actions explicitly not supported first: Azure CDN console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Cdn/profiles/endpoints/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `Caddy with Varnish cache` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/azure-cdn`.
...
- Error mapping: translate `azure/azure-cdn` backend auth, missing `azurerm_cdn_profile`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
