...
- Actions explicitly not supported first: Managed Disk console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Compute/disks/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `Longhorn block volumes` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/managed-disk`.
...
- Error mapping: translate `azure/managed-disk` backend auth, missing `azurerm_managed_disk`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
