...
- Actions explicitly not supported first: Azure VM console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Compute/virtualMachines/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `Incus or libvirt virtual machines` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/azure-vm`.
...
- Error mapping: translate `azure/azure-vm` backend auth, missing `azurerm_linux_virtual_machine`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
