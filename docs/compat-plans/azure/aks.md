...
- Actions explicitly not supported first: AKS console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.ContainerService/managedClusters/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `K3s or upstream Kubernetes` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/aks`.
...
- Error mapping: translate `azure/aks` backend auth, missing `azurerm_kubernetes_cluster`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
