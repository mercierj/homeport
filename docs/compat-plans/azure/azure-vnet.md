...
- Actions explicitly not supported first: Azure VNet console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `Microsoft.Network/virtualNetworks/read` and its paired read/list calls.
...
- Runtime/provisioning: provision `Cilium and Linux bridge networking` with the generated runtime manifest, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/azure-vnet`.
...
- Error mapping: translate `azure/azure-vnet` backend auth, missing `azurerm_virtual_network`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
