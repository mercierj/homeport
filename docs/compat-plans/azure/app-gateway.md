...
- Error mapping: translate `azure/app-gateway` backend auth, missing `azurerm_application_gateway`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
