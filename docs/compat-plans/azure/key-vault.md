...
- Runtime/provisioning: provision `HashiCorp Vault` with generated `artifacts/compat/azure/key-vault/backend.yaml`, health endpoint, persistence volume, backup job, endpoint route, and teardown script for `azure/key-vault`.
...
- Error mapping: translate `azure/key-vault` backend auth, missing `azurerm_key_vault`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
