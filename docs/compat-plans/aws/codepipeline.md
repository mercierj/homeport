...
- Actions explicitly not supported first: CodePipeline console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `codepipeline:CreatePipeline` and its paired read/list calls.
...
- Error mapping: translate `aws/codepipeline` backend auth, missing `aws_codepipeline`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
