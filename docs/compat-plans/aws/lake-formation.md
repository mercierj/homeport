...
- Actions explicitly not supported first: Lake Formation console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `lakeformation:GrantPermissions` and its paired read/list calls.
...
- Error mapping: translate `aws/lake-formation` backend auth, missing `aws_lakeformation_permissions`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
