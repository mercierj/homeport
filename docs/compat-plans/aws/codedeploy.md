...
- Actions explicitly not supported first: CodeDeploy console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `codedeploy:CreateApplication` and its paired read/list calls.
...
- Error mapping: translate `aws/codedeploy` backend auth, missing `aws_codedeploy_app`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
