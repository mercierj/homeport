...
- Actions explicitly not supported first: Step Functions console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `states:CreateStateMachine` and its paired read/list calls.
...
- Error mapping: translate `aws/step-functions` backend auth, missing `aws_sfn_state_machine`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
