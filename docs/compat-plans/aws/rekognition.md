...
- Actions explicitly not supported first: Rekognition console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `rekognition:CreateCollection` and its paired read/list calls.
...
- Error mapping: translate `aws/rekognition` backend auth, missing `aws_rekognition_collection`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
