...
- Actions explicitly not supported first: Textract console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `textract:StartDocumentAnalysis` and its paired read/list calls.
...
- Error mapping: translate `aws/textract` backend auth, missing `aws_textract_adapter`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
