...
- Actions explicitly not supported first: SNS console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `sns:CreateTopic` and its paired read/list calls.
...
- Error mapping: translate `aws/sns` backend auth, missing `aws_sns_topic`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
