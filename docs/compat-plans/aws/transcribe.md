...
- Error mapping: translate `aws/transcribe` backend auth, missing `aws_transcribe_vocabulary`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
