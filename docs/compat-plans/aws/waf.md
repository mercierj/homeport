...
- Actions explicitly not supported first: WAF console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `wafv2:CreateWebACL` and its paired read/list calls.
...
- Error mapping: translate `aws/waf` backend auth, missing `aws_wafv2_web_acl`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
