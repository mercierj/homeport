...
- Actions explicitly not supported first: CodeBuild console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `codebuild:CreateProject` and its paired read/list calls.
...
- Error mapping: translate `aws/codebuild` backend auth, missing `aws_codebuild_project`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
