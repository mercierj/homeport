...
- Actions explicitly not supported first: API Gateway console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `apigateway:CreateRestApi` and its paired read/list calls.
...
- Error mapping: translate `aws/api-gateway` backend auth, missing `aws_api_gateway_rest_api`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
