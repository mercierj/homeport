...
- Actions explicitly not supported first: RDS console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `rds:CreateDBInstance` and its paired read/list calls.
...
- Error mapping: translate `aws/rds` backend auth, missing `aws_db_instance`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
