...
- Initial supported surface: iam:CreateRole, iam:GetRole, iam:ListRoles, iam:UpdateRole, iam:DeleteRole.
- Actions explicitly not supported first: IAM console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `iam:CreateRole` and its paired read/list calls.
...
- Actions: iam:CreateRole, iam:GetRole, iam:ListRoles, iam:UpdateRole, iam:DeleteRole.
...
- Error mapping: translate `aws/iam` backend auth, missing `aws_iam_role`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
