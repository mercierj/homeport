...
- Actions explicitly not supported first: MQ console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `mq:CreateBroker` and its paired read/list calls.
...
- Error mapping: translate `aws/mq` backend auth, missing `aws_mq_broker`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
