...
- Actions explicitly not supported first: ALB console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `elasticloadbalancing:CreateLoadBalancer` and its paired read/list calls.
...
- Error mapping: translate `aws/alb` backend auth, missing `aws_lb`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
