...
- Actions explicitly not supported first: Athena console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `athena:CreateWorkGroup` and its paired read/list calls.
...
- Error mapping: translate `aws/athena` backend auth, missing `aws_athena_workgroup`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
