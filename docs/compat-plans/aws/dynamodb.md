...
- Actions explicitly not supported first: DynamoDB console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `dynamodb:CreateTable` and its paired read/list calls.
...
- Error mapping: translate `aws/dynamodb` backend auth, missing `aws_dynamodb_table`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
