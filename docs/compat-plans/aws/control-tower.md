...
- Actions explicitly not supported first: Control Tower console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `controltower:EnableControl` and its paired read/list calls.
...
- Error mapping: translate `aws/control-tower` backend auth, missing `aws_controltower_control`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
