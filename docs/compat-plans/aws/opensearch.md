...
- Actions explicitly not supported first: OpenSearch console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `es:CreateDomain` and its paired read/list calls.
...
- Error mapping: translate `aws/opensearch` backend auth, missing `aws_opensearch_domain`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
