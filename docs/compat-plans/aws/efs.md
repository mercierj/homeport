...
- Initial supported surface: efs:CreateFileSystem, efs:DescribeFileSystems, efs:UpdateFileSystem, efs:DeleteFileSystem.
- Actions explicitly not supported first: EFS console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `efs:CreateFileSystem` and its paired read/list calls.
...
- Actions: efs:CreateFileSystem, efs:DescribeFileSystems, efs:UpdateFileSystem, efs:DeleteFileSystem.
...
- Error mapping: translate `aws/efs` backend auth, missing `aws_efs_file_system`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
