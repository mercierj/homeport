...
- Actions explicitly not supported first: CloudWatch console-only workflows, account billing, quota purchase flows, and managed cross-region failover controls outside `cloudwatch:PutMetricAlarm` and its paired read/list calls.
...
- Error mapping: translate `aws/cloudwatch` backend auth, missing `aws_cloudwatch_metric_alarm`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider-shaped access-denied/not-found/conflict/validation/throttle/internal-error responses with retry hints.
