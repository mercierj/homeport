...
- Actions explicitly not supported first: IoT Core console-only workflows, commercial billing/quota administration, provider-managed fleet automation, and cross-region control-plane features outside `iot:CreateThing` and its paired read/list calls.
...
- Error mapping: translate `aws/iot-core` backend auth, missing `aws_iot_thing`, duplicate import, malformed request, timeout, quota, and dependency failures to the provider error families above with retry hints.
